...
static uint32_t MessageCenterEnterCritical(void)
{
#ifdef USE_FREERTOS
    taskENTER_CRITICAL();
    return 0U;
...
static void MessageCenterExitCritical(uint32_t state)
{
#ifdef USE_FREERTOS
    (void) state;
    taskEXIT_CRITICAL();
...
static uint8_t CheckName(const char *name)
{
    if ((name == NULL) ||
...
Publisher_t *PubRegister(char *name, uint8_t data_len)
{
    ...
    while (node->next_topic_node) // message_center会直接跳过,不需要特殊处理,这被称作dumb_head(编程技巧)
    {
        ...
        {
            if (!CheckLen(data_len, node->data_len)) {
                return NULL;
            }
    ...
    // 在链表尾部创建新的话题并初始化
    node->next_topic_node =
        (Publisher_t *)BSPMalloc(sizeof(Publisher_t));
...
Subscriber_t *SubRegister(char *name, uint8_t data_len)
{
    ...
    Subscriber_t *ret = (Subscriber_t *)BSPMalloc(sizeof(Subscriber_t));
    if (ret == NULL) {
        return NULL;
    }
    ...
    { // 给消息队列的每一个元素分配空间,queue里保存的实际上是数据执指针,这样可以兼容不同的数据长度
        ret->queue[i] = BSPMalloc(data_len);
        if (ret->queue[i] == NULL) {
...
uint8_t SubGetMessage(Subscriber_t *sub, void *data_ptr)
{
    ...
    uint32_t critical_state = MessageCenterEnterCritical();
    if (sub->temp_size == 0U) {
        MessageCenterExitCritical(critical_state);
        return 0;
    }

    memcpy(data_ptr, sub->queue[sub->front_idx], sub->data_len);
    sub->front_idx = (sub->front_idx + 1U) % QUEUE_SIZE;
    sub->temp_size--;
    MessageCenterExitCritical(critical_state);
    return 1;
}
