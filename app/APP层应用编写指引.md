...
这是机器人的公共定义和参数模板文件，必须要针对每个机器人进行修改。包括机器人的尺寸参数、性能参数、视觉链路和需要启用的app等。当前MSPM0G3507_BASIC工程只保留单主控方案，不再维护板型切换宏。
...
机器人命令模块是对整个机器人的抽象，该应用应该包含接收控制指令的模块，例如遥控器、视觉通信模块。该模块会处理接收到的控制数据，并将其转化为**具体的、定量的**控制信息，发送给其他模块。同时，cmd应用会处理模块和应用离线的情况，出现紧急状况时停止所有执行机构的运行。
...
以步兵为例，底盘应该包括4个电机。根据cmd应用发来的控制信息，进行麦克纳姆轮的运动学解算，从而获知四个电机需要的设定值，然后调用电机提供的接口进行设定。chassis还要根据电机的反馈数据以及IMU信息（如果有），计算底盘的实际运动状态，反馈给robot_cmd应用。
...
当前工程按单主控迁移。不同机器人可以继续在`robot_def.h`中保留共享结构体和参数模板，但不要再通过板型宏拆分app逻辑。
