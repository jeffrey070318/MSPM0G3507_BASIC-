...
当前MSPM0G3507_BASIC工程只保留单主控方案，不再维护板型条件编译。应用之间的通信统一按本工程的消息中心/任务接口重新接入。
...
application在初始化module的时候，初始化参数会包含部分bsp的内容，但仅仅是SysConfig生成的外设实例、引脚选择和id设置。外设的时钟、引脚和功能归属以 `MSPM0G3507_BASIC.syscfg` 为准，app/module只引用生成的实例宏，不直接依赖底层初始化代码。
